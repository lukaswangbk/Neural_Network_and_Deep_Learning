...
class ImageGenerator(object):
    def __init__(self, x, y):
        ...
        self.translated_pixels_num, self.rotation_degree = 0, 0
        self.is_horizontal_flip, self.is_vertical_flip, self.is_add_noise, self.is_bright = False, False, False, False
    ...
    def next_batch_gen(self, batch_size, shuffle=True):
        ...
        batch_count = 0
        total_batch = self.N_aug // batch_size
        ...
        while True:
            if (batch_count < total_batch):
                ...
                if shuffle:
                    index = np.random.permutation(self.N_aug)
    ...
    def translate(self, shift_height, shift_width):
        ...
        self.translated_pixels_num = self.height*self.width - (self.height-shift_height)*(self.width-shift_width)
        translated = np.roll(self.x.copy(), (shift_height, shift_width), axis=(1,2))
    ...
    def rotate(self, angle=0.0):
        ...
        rotated = rotate(self.x.copy(), angle=angle, axes=(1,2), reshape=False)
    ...
    def add_noise(self, portion, amplitude):
        ...
        index = np.random.choice(self.N, int(self.N * portion), replace=False)
        added = self.x[index].copy() + int(amplitude * float(np.random.rand(1)))
        
        self.added = (added, self.y[index].copy())
        self.N_aug += int(self.N * portion)
        print("Noise added by portion: ", portion, "amplitude: ", amplitude)
        return added
