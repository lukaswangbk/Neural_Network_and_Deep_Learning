...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            params[k] -= learning_rate / (np.sqrt(velocitys_hat) + eps) * momentums_hat
        self.t = t
...
class NadamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            momentum_hat = momentums[k] / (1 - np.power(beta1, t))
            velocity_hat = velocitys[k] / (1 - np.power(beta2, t))
            params[k] -= learning_rate / (np.sqrt(velocity_hat) + eps) * \
                            (beta1 * momentum_hat + \
                             (1 - beta1) * grads[k] / (1 - np.power(beta1, t)))
        self.t = t
